...
components/webgl/       ogl/WebGL shader + image-sequence components.
                        ...
                        (no WebGL — see "SceneUnfold" below) and is generic
                        enough to reuse for SceneRail.
...
part of `page.tsx`. `Hero` and `SceneUnfold` are fully built (see "Hero
WebGL" and "SceneUnfold" below). `SceneRail`, `Manifesto`, `TheDrop`, and
`ShopGrid`/`Footer` are still bare placeholder stubs in
`components/sections/` (brand tokens applied, no real animation/data-
fetching logic yet). `SceneRail` is next up to reach into `components/webgl`
(alongside `Hero` and `SceneUnfold`, which already do) for its own
scroll-driven scene — reuse `SceneUnfoldCanvas`'s pattern (or the component
itself, it's already generic: `images` + a `getProgress` callback) rather
than rebuilding sequence-frame drawing from scratch.
